...
def chat(req: ChatRequest, request: Request, db: Session = Depends(get_db)):
    ...
    db_session = db.query(ChatSession).filter(ChatSession.session_id == client_ip).first()
    if not db_session:
    ...
    try:
        ...
        db.rollback()
        app_logger.error(f"Database write failed during /chat transaction: {db_err}", exc_info=True)
